...
def lambda_handler(event, context):
    ...
    if os.environ['TargetNodeGroupTagKey'] :
        ...
        log("Empty Environment variable TargetNodeGroupTagKey:"+ os.environ['TargetNodeGroupTagKey'])
        exit (1)
    ...
    for rsv in reservations:
        for instance in rsv['Instances']:
           ...
           inst_placment=instance.get("Placement")
           inst_az=inst_placment['AvailabilityZone']
           for k in inst_tags:
               if k['Key'] == target_ng_tag_key:
                   tag_matched = True
    if tag_matched == False:
        log("nodegroup tag doesn't match, exit...")
        exit (1)
    ...
    if os.environ['Az1SubnetIds'] :
        az1_subnet_ids = os.environ['Az1SubnetIds'].split(",")
    else:
        log("Empty Environment variable SubnetIds:"+ os.environ['Az1SubnetIds'])
        exit (1)
    if os.environ['Az2SubnetIds'] :
        az2_subnet_ids = os.environ['Az2SubnetIds'].split(",")
    else:
        log("Empty Environment variable SubnetIds:"+ os.environ['Az2SubnetIds'])
        exit (1)

    ## Find AZ: Check instance's primary interface to find AZ
    subnets1 = ec2_client.describe_subnets(SubnetIds=[az1_subnet_ids[0]]).get("Subnets")
    subnets2 = ec2_client.describe_subnets(SubnetIds=[az2_subnet_ids[0]]).get("Subnets")
    subn_matched = False
    if subnets1 != None:
        subn1 = subnets1[0]
        if subn1['AvailabilityZone'] == inst_az:
            subnet_ids = az1_subnet_ids
            subn_matched = True
            log("instance primary subnet matches subnet list input group1")
    if subnets2 != None:
        subn2 = subnets2[0]
        if subn2['AvailabilityZone'] == inst_az:
            subnet_ids = az2_subnet_ids
            subn_matched = True
            log("instance primary subnet matches subnet list input group2")

    ## Sanity check for input subnet list: check all subnets again before further proceed
    if subn_matched == False:
        log("AZs don't match between input subnets' AZ and instance's AZ")
        exit (1)
    else:
